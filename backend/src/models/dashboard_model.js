...
export const getShippingScheduleTimeline = async () => {
  const [rows] = await dbPool.query(
    "SELECT vessel_code, vessel_name, eta_date, latest_berthing, target_load_tons FROM shipping_schedule ORDER BY eta_date ASC"
  );
  return rows;
};
...
export const getTargetVsActualLoadingPerVessel = async () => {
  const [rows] = await dbPool.query(
    `SELECT ss.vessel_code,
     ...
     GROUP BY ss.vessel_code, ss.vessel_name, ss.target_load_tons, ss.eta_date
...
export const getVesselStatusList = async () => {
  const [rows] = await dbPool.query(
    "SELECT vessel_code, vessel_name, loading_status FROM shipping_schedule"
  );
  return rows;
};
